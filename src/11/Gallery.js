...
export default function Gallery() {
  const [tdata, setTdata] = useState([]); //키워드에 따라 카드를 다시 만들어주는
  const [tags, setTags] = useState([]); //패치로 tdata가 만들어진 후
  ...
  const getFetchData = async() => {
    ...
    url = `${url}serviceKey=${serviceKey}&numOfRows=20&pageNo=1&MobileOS=ETC&MobileApp=AppTest&arrange=A&keyword=${keyword}&_type=json`;
  ...
  useEffect(()=>{
    const tm = tdata.map(item => <TailCard key = {item.galContentId}
  ...
  return (
    <div className="w-full flex flex-col justify-center items-center">
      ...
      <div className="w-10/12 p-2 grid grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-2">
